...
Note, when I say two variables - I mean any predictor (x) with a given outcome variable (). We can have many X's, but the regression models are giving us information about each X in its relationship with y - not to each other.
...
We can actually unpack this difference in equations! Here is a linear regression *Without* moderatotion for instagram, self esteem, and gender:

$$SelfEsteem = \beta_0 + \beta_1 \times InstagramUse + \beta_2 \times Gender$$
Now, with a moderator - we add one more term with the X variables interacting. Literally, we multiple them together and fit abother beta coefficient.

$$SelfEsteem = \beta_0 + \beta_1 \times InstagramUse + \beta_2 \times Gender + \beta_3 \times InstagramUse \times Gender$$
...
Interesting. For one, we can see that the r-squared has increased to about .44 from .36 in the multiple regression model--This model seems to fit the data much better with an added interaction. It also looks like once we add the interaction to the model, the unique effect of `female` becomes non-significant, and the unique effect of `insta` actually *flips* to be positive. Why all this change? Well, once the variance was correctly attributed to the interaction between the variables rather than each of their linear effects, it seems that we now have a clearer picture of the true unique linear effect of each variable. This is good to remember: when we model regressions, we get the unique effect of each variable *controlling for the other variables in the model*, not anything we haven't included, such as potentially important interaction terms. Without the controls - we were getting bias from the ommited variables. This is unsuprisingly called *Omitted Variable Bias* or *OVB* for short. 
...
Conversely, we can get pretty much the same visuale just using `ggplot` - so if you're a die hard ggplotter - I am, something like this works too.
...
Now let's look at a little visualization to compare centered versus not centered data - that way we can see that nothing really changed besides a shift! we will just use the grit compared to gpa data with the centered grit schores
...
```
As you can see, this is jsut a horizontal shift! The data is the same shape - which will preserve our analysis.
...
3.1 The effect of x is lessened by the inclusion of m.
...
THAT is moderation - it's kind of like regression-inception (yes I made that term up and it's brilliant) - regression within a regression!!
...
1. Select a random sample **with replacement** from your data. This implies that some cases might be selected more than once, and some others might not be selected at all. We use **with replacement** so we can get a large enough representativee simple that isn't exactly the same as our sample.
...
We wont go into much detail here, but it is nice to start getting familiarized with this package and its syntax, because you will likely use it if you move into these sorts of analyses. It's kind of annoying at first, but it gets much easier with aa little practice.
...
- R-Square: We get how much variance is explained for the mediator and the outcome. We explain 6.5% of the variance in `depress2` and 0.2% of the variance in `job_seek`. Nkt a lot lol.
