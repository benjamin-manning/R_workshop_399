---
title: "R Workshop 71: Mediation and Moderation"
...
Well, it was a lot - that's probably what you remember the most. To get it into your head - he's a brief summary!

We discussed correlation and regression--two ways to assess the linear relationships between two or more variables. Here's what we covered:
...
Hey Ho, LET'S GOOOOOOOOO!! 

Wait, I'm realizing that it's been a super long time since I told you a joke, and obviously you're missing them. Sooooo ya:
...
We can actually unpack this difference in equations! Here is a linear regression *Without* moderation for instagram, self-esteem, and gender:

$$SelfEsteem = \beta_0 + \beta_1 \times InstagramUse + \beta_2 \times Female$$
Now, with a moderator - we add one more term with the X variables interacting. Literally, we multiple them together and fit another beta coefficient.

$$SelfEsteem = \beta_0 + \beta_1 \times InstagramUse + \beta_2 \times Female + \beta_3 \times InstagramUse \times Female$$
That's the only mathematical difference! Not too complicated - yay.
...
So, how can we test the moderation of gender, to see if there are differential effects of instagram use across gender? It's pretty simple, actually: simply replace the `+` with an asterisk (`*`):
...
So, we can understand moderation as a *multiplicative*, rather than an *additive* effect. Multiple regression only allows for additive, individual relationships with the outcome. Moderation allows for multiplicative, multi-variable relationships with the outcome. And the shortcut R syntax `*` represents that multiplicative nature well, while also being the most concise way (least amount of code) to model moderation, too. So that's great--thanks, R :). At the end of the day - these will give the EXACT same mathematical output - so if you're confused by one, use a different one and just make sure that they give the same result.

OK, let's return to our concise code and see what results this model gives us:
...
For reference, I want you to try to interpret the coefficients in the moderated example. During our next session - we will discuss them in person.

Interesting. For one, we can see that the r-squared has increased to about .44 from .36 in the multiple regression model--This model seems to fit the data much better with an added interaction. It also looks like once we add the interaction to the model, the unique effect of `female` becomes non-significant, and the unique effect of `insta` actually *flips* to be positive. Why all this change? Well, once the variance was correctly attributed to the interaction between the variables rather than each of their linear effects, it seems that we now have a clearer picture of the true unique linear effect of each variable. This is good to remember: when we model regressions, we get the unique effect of each variable *controlling for the other variables in the model*, not anything we haven't included, such as potentially important interaction terms. Without the controls - we were getting bias from the omitted variables. This is unsurprisingly called *Omitted Variable Bias* or *OVB* for short. 

**Definition:** omitted-variable bias occurs when a statistical model leaves out one or more relevant variables. The bias results in the model attributing the effect of the missing variables to those that were included.
...
While not always necessary, especially when just using visualization for our own interpretation, adding a little extra effort into the graph to make it more readable is great for presenting, and easy to do with `interact_plot`.

Conversely, we can get pretty much the same visual just using `ggplot` - so if you're a die hard ggplotter - I am, something like this works too.
...
sm %>% 
  ...
  labs(title='Black line is original regression without moderation')
```
...
Yes we can!!!! AYYYYYYYYYYOOOOOO. Honestly, we can do anything with the power of love! JK - the power of stats. STATS IS ALL OMNIPOTENT.
...
```
Once again, I want you to try to interpret the coefficients in the moderated example. Write down what you think they mean! During our next session - we will discuss them in-person and I will unpack them below.
...
Recreating this plot in `ggplot` is tougher Continuous moderation plots are a RARE example where I would implore you to bail on `ggplot`. This requires manual entry from coefficient outputs and is annoying, so I'm going to spare you the trouble and say just don't do it lolz.

### Mean centering 

We've learned how to assess categorical and continuous moderators, but we need to consider one more thing to complete our mastery of moderation. Then, I can knight you as a MOD MASTER!!

The astute statistician (you and me obvi) may be wary of our moderation models up to this point. Why? Because being well aware that linear regression assumes no multicollinearity (near perfect correlation) between variables, they are suspicious of the fact that we added information from the same variables into our model twice. For example, with our last model, we added information from `grit` and `age` in as individual predictors, but also when we added the `grit*age` interaction. Regression doesn't work well when we add multiple variables that include very similar information--it becomes very difficult to identify the unique effects of each variable. Is too-high-correlation a problem here, though? Let's see: First, let's create our interaction term and then see how much it correlates with each of our predictors.
...
Once we mean center our variables, the order of the data is the same (i.e. those who had high scores before still have high scores and vice versa), but they are no longer highly correlated to the moderation term. 
...
Now let's look at a little visualization to compare centered versus not centered data - that way we can see that nothing really changed besides a shift! we will just use the grit compared to gpa data with the centered grit scores

```{r}
#ignore this code - just doing a little trick to make the graph pretty.
grit_graph <- data.frame(append(grit$grit, grit$grit_c))
grit_graph$gpa <- append(grit$gpa,grit$gpa)
grit_graph$grit <- grit_graph$append.grit.grit..grit.grit_c.

grit_graph <- grit_graph %>% 
  mutate(centered = 1)

grit_graph$centered[473:944] <- 0


grit_graph %>%
  ggplot() +
  geom_point(aes(x = grit, y= gpa, color = factor(centered)))+
  scale_color_discrete(labels = c(paste("Centered, mean = ",0),
                                  paste("Original, mean = ", round(mean(grit$grit),2)))) + 
  labs(color = 'Data')

```
As you can see, this is just a horizontal shift! The data is the same shape - which will preserve our analysis.

Let's see if this solved our problem with the correlations:
...
If we look at the plot again, we see that the slopes haven't changed--our effects are the same, we have just manipulated how our variables are expressed (by mean-centering) to reduce multicollinearity and ease interpretation of some coefficients. WOOOOT. MAGICAL STATS. #STATSARECOOL #CENTERINGISCOOL #EFFECTSARECOOL #MODERATAIONISCOOL #BENISCOOL (the last one is especially true....)
...
After running this experiment, we might be interested in understanding why this is. Maybe when people take notes manually, they tend to translate the information in their own words, rather than copying textually from the lecture. Maybe the amount of verbatim text is a good proxy for the depth with which students are processing the information, and *that* is what explains the effect. This would be visualized in the following way.
...
Mediation is vital for explanation. To explain a phenomenon means to provide a cause that accounts for it. Even better, a more complete explanation provides a causal chain, linking the phenomenon with proximate causes, and explaining these proximate causes by more distal ones. Mediation is a statistical tool that allows us to look at these causal links. In statistical parlance: It allows us to know that the relationship between an independent variable (X) and the dependent variable (Y) is explained by a mediator (M).

All this talk about causality might make you think that the use of certain statistical techniques allows you to prove that X *causes* Y. This is a common misconception. Causality is addressed by experimental design, NOT by statistics (this isn't 100% true - but you should operate under the assumption that it is!). If we want to say that something causes something else, we must make sure that at least three conditions are met:

1. The cause precedes the consequence.
2. The cause and the consequence co-occur, they are correlated.
3. Alternative explanations can be ruled out.

I could collect self-report data on trait neuroticism, state anxiety, and a current diagnosis of an anxiety disorder cross-sectionally and propose the following model.

![Arrows in a picture are not proof of causality](Picture3.png)

However, without longitudinal data that shows what came first, without a way to rule out reverse causality and alternate explanations, we shouldn't be so quick to assume causality just because we have a fancy statistical model and a pretty picture to go with it (even though I love pretty pictures! <3). Maybe the existence of a disorder increases state anxiety and makes people self-report their traits differently. Maybe there is a different cause, say genetic in nature, that explains the three collected variables.

So always remember, when conducting or reading research that uses mediation, make sure that there are good reasons to assigning each variable as a predictor, criterion or mediator.

## The traditional way to do mediation
...
4. The effect of x is lessened by the inclusion of m.
5. Finally, inverting the regression, we should see that our outcome does *not* predict our predictor, controlling for the mediator `x ~ y + m`.
...
But wait, timeout - what is mediation? It's still confusing! Mediation is when you run a regression of some variable on another, and then use the predicted values of the first regression (often signified with a ^ above the variable) with the first variable on a 3rd variable to get results!!! We can use `age`, `grit`, and `gpa` from the moderation example to get some help! I think that `age` predicts `gpa`, but it's also mediated by `grit`! I made this up - it's just an example. Here are the equations:

First regression, which would be `m ~ x`: $Grit = \beta_0 + \beta_1Age + \epsilon_i$

Now we use the predicted `grit` values along with the `age` values to predict `gpa`. For reference, this is the nomenclature of predicted versus original variables:

Original: $Grit$
Predicted $\widehat{Grit}$

Okay, so this would be thee final mediated regression `y ~ x + m`: $GPA = \beta_0 + \beta_1Age + \beta_2\widehat{Grit} + \epsilon_i$

THAT is mediation - it's kind of like regression-inception (yes I made that term up and it's brilliant) - regression within a regression!! If you haven't seen the movie inception - that's officially your homework this week. I'm not going to say it was that good; but it was such a cultural phenomenon that I think one is obligated to watch it. Also, then you'll you get the joke I just made ;)

We could theoretically repeat this process a TON of times to get super complicated mediators with like 20 regressions inside the final one. For learning purposes, we aren't going to worry about this for now!
...
## Bootstrapped confidnece intervals

This is a method to determine if we want to be using mediators! Do they matter? We have a test for that - duh. There's a test for that. There's an app for that!
...
1. Select a random sample **with replacement** from your data. This implies that some cases might be selected more than once, and some others might not be selected at all. We use **with replacement** so we can get a large enough representative simple that isn't exactly the same as our sample.
...
5. If the 95% confidence intervals do not include 0, we can say there is a significant effect.
...
2. Calculate the direct and indirect effects for each of the 1000 random samples and look at the 95% confidence intervals. A note: The direct effect is x's coefficient in the final regression on y in `y ~ m + x`. The indirect effect is effect through the mediator. So it's the coefficient of x in `m ~ x` times the coefficient of m in `y ~ m + x`.

In pictures, using a previous image:
...
Basically, this entire output is just permutations of the beta coefficients combined in different ways!
...
3. Looking at a plot might help us interpret these better. If the horizontal line (the confidence interval) intersects the vertical line (0), then the effect is not significant.

```{r}
plot(mediation)
```
So, maybe using a mediator here was unnecessary - all we needed was normal regression. However, now you know how to figure that out!!
...
```{r}
#if you can't figure this out, we will go over it during our next meeting! :)
```

It seems like adding these covariates does not make the effects significant (that's your hint to see if you got it right ;)).

## Mediation in `lavaan`

Another way to run mediation is to use package `lavaan`. This package allows you to run path models and structural equation models (translation: fancy mediation models), which are very useful for many reasons. Among them are:
- You can estimate multiple regressions simultaneously
...
We won't go into much detail here, but it is nice to start getting familiarized with this package and its syntax, because you will likely use it if you move into these sorts of analyses. It's kind of annoying at first, but it gets much easier with aa little practice.

lavaan's `sem()` - stands for "structural equation model", function works pretty much in the same way as `lm()`. We first specify the model (akin to lm's formula) and then provide the data.
...
For mediation we primarily care about the regression and parameter defining operators. The full model goes in between single quotes (').
...
- R-Square: We get how much variance is explained for the mediator and the outcome. We explain 6.5% of the variance in `depress2` and 0.2% of the variance in `job_seek`. Not a lot lol.
...
### Practice. Mediation with covariates.
As in the prior example, write out the syntax for a model that includes `age`, `sex`, and `econ_hard` as covariates. Once again, we will cover this together next time we meet!
...
Technically, moderation is done by running regression with a multiplicative term between two predictors. Moderation is done by comparing results from two nested regression models.
...
1. **Moderation**
  - Moderation using `lm`
  - Moderation for both continuous and categorical variables
  - Centering in moderation
  - Interpreting moderation
2. **Mediation**
  - Regression-Inception ;)
  - Bootstrapping
