...
In the last tutorial, we started our ggplot adventure. Was it more fun than bringing the ring to Mordor? Definitely. More specifically, we dived into multivariate plots with geom_point, geom_col and geom_line()
...
**Visualization**
  + simple plots with different geoms
...
```
Hey, what rhymes with geom? Seriously, I can't think of anything while I'm writing this, so please tell me. Big gnome? Go home? Capital Ohm? The city Rome? kinda...
...
Here are some other geoms for descriptive plots. Play around with some stuff! Change lots of the variables to figure out what's going on. Add stuff, take things away; be free my child. Also, make sure you understand the plots themselves - creating legible plots is a statistician's greatest skill.
...
----------------------|---------------------------------------------------------------------
`color =`             | Separates groups by color of obs
`shape =`             | Separates groups by shape of obs
`size =`              | Separates groups by size of obs
...
Here are some common aesthetics you might try using. As you see below, if you apply aesthetics as global aesthetics in `ggplot()` they will apply to all layers. As we saw, changing features outside of `aes()`, though, (e.g. color), need to be specified in each layer. Remember, color can be edited inside or outside an aesthetic, but his has a different outcome!
...
Also notice that not all aesthetics work for all types of geoms--"shape", for example, doesn't work with `geom_smooth`, so the global aesthetic `shape = gear` ignores that layer. You can, however, add another aesthetic that *does* apply to `geom_smooth()`, such as "linetype". I've said this before and I'll say it again: use `?` to find out more about any function! All geoms have a list of their acceptable aesthetics if you search them.
...
Sometimes, you want to BREAK DOWN and cry after a long day - I know I do. However, sometimes, you also want to BREAK DOWN a graph based on groups. When using colors or other graphical devices where it's not possible (they are already mapped to other variables) or the resulting graph would look too crowded, you can use faceting to repeat the graph subset by each level of the faceting variable. This can be both a final data visualization step or an intermediary step for you to get your bearings!
  
```{r}
#NOTE - FACET_GRID AND FACET_WRAP can be "weird" sometimes and use different syntax than other ggplot items. If you're struggling to use them, do the help function or just google examples :)
...
mtcars %>% #Cars Dataframe
  rownames_to_column("Name") %>%  #Makes the names of the car a variable
...
Now take out the coord_flip() argument and see what happens. Welp - that was a mess! We can see why coord_flip() is a nice little trick

What if you want to zoom in on a plot. Inside the coord_cartesian function, you can use the xlim and ylim arguments to specify what ranges of data you want to show. Here is an example.
...
```
Zooming in is particularly useful when there are a few misleading outliers in your data and they screw up the axes!
...
Now let's add scale_y_log10, and see how that improves the plot (another way to deal with strange outliers our scale problems)
...
Corona %>% 
  ...
  scale_y_log10()
```
Look at that! Taking the log of the y-axis gives us a different picture of the plateaus and movements of the pandemic. You've probably seen a lot of covid graphs over the past year - always make sure you know that the axes are before interpreting a visual.
...
Of course, as would be true for any good graph, we can add labels for the x and y axes (we are so fancy!!):
...
```
Damnnnn that graph is looking sexy! Can't stop us plotting now. Can't stop won't stop...
...
`theme()` can also be used to tweak aspects of your graph's theme manually. Check these out! Pick your fave and remember it whenever you want to graph like a boss.
...
That was a LOT. My goodness. Check out everything you did!
