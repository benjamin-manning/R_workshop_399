...
exp <- read.csv("experiment.csv", header = TRUE)
...
```{r}
exp %>%
  select(gender,se1:se8) %>% 
  gather(item,response,-gender) %>% 
  group_by(gender,item) %>% 
  summarise(item_mean = mean(response),item_sd = sd(response))



#If you want the recoded items
exp %>% 
  select(gender,se1,se2,se3_r,se4,se5_r,se6,se7_r,se8) %>% 
  gather(item,response,-gender) %>% group_by(gender,item) %>% 
  summarise(item_mean = mean(response),item_sd = sd(response))
