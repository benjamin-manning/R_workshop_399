...
Simply running a line of code that does not have an equals sign will print output in R markdown. However, you can also use `print()`. For the moment, we will consider these equivalent and can be used interchangeably. As you can see below, the outputs are thee same when you run the chunk of code.
...
We can also use more complex names to store numbers using `.` or `_` as mentioned earlier. There is no universal standard for `.` or `_`, but I general prefer the underscore as it's easier to read. Below are examples of both! Remeber, jsut don't use a real space!
...
## A brief Forray into Data types
